...
// Node comparable comparison function
...
// Node ruleset execution function
				...
				if (nodeComparableCompare(sourceNodeModules[nodeRuleSet[i].sourceNodeModule].state[sourceNodeComparableField], nodeRuleSet[i].sourceNodeComparableOperator, nodeRuleSet[i].sourceNodeComparableValue)) {
					...
					if (sourceNodeModules[nodeRuleSet[i].sourceNodeModule].state.status !== sourceNodeModules[nodeRuleSet[i].sourceNodeModule].lastState.status ||
						...
						if (destNodeIP !== null) {
							...
							console.log(util.inspect(nodeRuleSet[i], {
								showHidden: false,
								depth: null,
								colors: true
							}));
...
// Main UDP announcement function
...
var udpAnnounce = function(moduleName) {
	...
	nodeModuleNames = Object.keys(nodeModules);
	...
	udpMessage = new Buffer(JSON.stringify(udpAnnounceMessage));
...
// Handle UDP message event
var onUDPMessage = function(msg, rinfo) {

	var message;
	var messageModuleList;
	var messageModuleListKeys;
	...
	messageModuleList = message.messageData.modules;
	messageModuleListKeys = Object.keys(messageModuleList);
	...
	if (message.messageType === 'announce' || message.messageType === 'state') {
		...
		for (var i = 0, j = messageModuleListKeys.length; i < j; i++) {
			...
			nodeList[rinfo.address].modules[messageModuleListKeys[i]].lastState = {
				status: null,
				data: null
			};
	...
	if (message.messageType === 'state') execRuleSet(rinfo.address, messageModuleListKeys[0]);
	else execRuleSet(rinfo.address);

};

// Handle UDP post-bind setup
var udpServerPostBind = function() {
	...
	if (req.method === 'OPTIONS') {
			...
			req.on('end', function() {
				...
				fs.writeFile('./users.json', JSON.stringify(users), function(err) {
					...
					if (err) {
						...
						res.writeHead(200, headers);
						res.write('Error updating users file!');
		...
		if (req.method === 'GET') {
			...
			req.on('end', function() {
				...
				fs.writeFile('./config.json', JSON.stringify(jsonPostData), function(err) {
					...
					// TODO: need to fix - and unload previous modules
		...
		if (req.method === 'GET') {
			...
			req.on('end', function() {
				...
				commandName = jsonPostData.commandName || '';
				commandData = jsonPostData.commandData || {};
				...
				for (var i = 0, j = commandParameters.length; i < j; i++) {
					...
					if (typeof commandData[commandParameters[i].name] === 'undefined') {
						...
						if (commandParameters[i].required) {
							...
							res.write(util.inspect(commandParameters[i], {
								showHidden: false,
								depth: null,
								colors: true
							}));
...
// Handle loading all modules found in config file
...
// Handle loading node configuration file
var loadNodeConfig = function() {
	...
	else if (typeof nodeConfig.deviceName === 'undefined') {
...
// Handle loading and validating the ruleset file for this node
...
// Handle load node device type file
var loadDeviceTypes = function() {
...
// Handle udp listening event
udpServer.on('listening', onUDPListening);

// Handle incoming UDP multicast messages
udpServer.on('message', onUDPMessage);

// start UDP server
udpServer.bind(udpPort, udpServerPostBind);
...
// Load saved data from filesystem
loadNodeConfig();
loadNodeModules();
loadUsers();
loadRuleSet();
